...
isCombiningOrControlChar char =
  isCombiningChar char || isControl char
    -- nb. Not sure if 'isControl' is the same as scala 'Character.isISOControl'

isCombiningCommand :: String -> Bool
isCombiningCommand = flip Map.member combining

translateCombining :: String -> String -> Either String String
translateCombining command str =
  case Map.lookup command combining of
...
makeNot negated =
  let s = case trim negated of
            ""      -> " "
            trimmed -> trimmed
  in case Map.lookup s Helper.Unary.not of
            Just r  -> r
...
tryMakeSubscript str =
  ...
  else traverse (flip Map.lookup subscripts) str
...
tryMakeSuperscript str =
  case str of
    "" -> Just ""
    s  -> traverse (flip Map.lookup superscripts) s
...
styles :: Map String (Map Char String)
styles = Map.fromList
  [ ("\\mathbb", bb)
  , ("\\textbb", bb)
  , ("\\mathbf", bf)
...
names = Set.fromList
  ...
  <> (Set.fromList $ Map.keys combining)
  <> (Set.fromList $ Map.keys styles)
